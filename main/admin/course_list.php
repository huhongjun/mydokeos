...
function get_number_of_courses()
{
	...
	elseif (isset ($_GET['keyword_code']))
	{
		...
		$keyword_courseTimes = Database::escape_string($_GET['course_times']);//zml add
		$sql .= " WHERE code LIKE '%".$keyword_code."%' AND title LIKE '%".$keyword_title."%' AND category_code LIKE '%".$keyword_category."%'  AND course_language LIKE '%".$keyword_language."%'   AND visibility LIKE '%".$keyword_visibility."%'    AND subscribe LIKE '".$keyword_subscribe."'AND unsubscribe LIKE '".$keyword_unsubscribe."'"."'AND course_times LIKE '".$keyword_courseTimes."'";
	}
...
function get_course_data($from, $number_of_items, $column, $direction)
{
	...
	$sql = "SELECT code AS col0, visual_code AS col1, title AS col2, course_language AS col3, category_code AS col4, subscribe AS col5, unsubscribe AS col6, code AS col7, tutor_name as col8, code AS col9, visibility AS col10, course_times AS col11 FROM $course_table";
	if (isset ($_GET['keyword']))
	{
		$keyword = Database::escape_string($_GET['keyword']);
		$sql .= " WHERE title LIKE '%".$keyword."%' OR code LIKE '%".$keyword."%'";
	...
	while ($course = Database::fetch_row($res))
	{
		...
		$course_rem = array($course[0],$course[1],$course[2],$course[3],$course[4],$course[5],$course[6],$course[7],$course[8],$course[9],$course[11]);
		$courses[] = $course_rem;
	}
	return $courses;
}
...
else
{
	...
	$table->set_header(10, get_lang('CourseTimes'));//zml add 
