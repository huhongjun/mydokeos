...
*/
	...
	function get_personal_session_course_list($user_id)
	{
		...
		//edit by xiaoping 
		$personal_course_list_sql = "SELECT course.code k, course.directory d, course.visual_code c, course.db_name db, course.title i, course.course_times cts,course.tutor_name t, course.course_language l, course_rel_user.status s, course_rel_user.sort sort, course_rel_user.user_course_cat user_course_cat
		...
		if(api_is_allowed_to_create_course())
		{
			foreach($sessions as $enreg)
			{
				$id_session = $enreg['id'];
				//edit by xiaoping
				$personal_course_list_sql = "SELECT DISTINCT course.code k, course.directory d, course.visual_code c, course.db_name db, course.title i, course.course_times cts,CONCAT(user.lastname,' ',user.firstname) t, email, course.course_language l, 1 sort, category_code user_course_cat, date_start, date_end, session.id as id_session, session.name as session_name
