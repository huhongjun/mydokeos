...
class Blog
{
	...
	function display_post($blog_id, $post_id)
	{
		...
		if(api_is_allowed('BLOG_' . $blog_id, 'article_delete', $task_id))
			$blog_post_actions .= '<a href="blog.php?action=view_post&amp;blog_id=' . $blog_id . '&amp;post_id=' . $post_id . '&amp;do=delete_article&amp;article_id=' . $blog_post['post_id'] . '&amp;task_id=' . $task_id . '" title="' . get_lang('DeleteThisArticle') . '" onclick="javascript:if(!confirm(\''.addslashes(htmlspecialchars(get_lang("ConfirmYourChoice"),ENT_QUOTES,$charset)). '\')) return false;"><img src="../img/delete.gif" border="0" /></a>';
	...
	function display_task_list($blog_id)
	{
		...
		if(api_is_allowed('BLOG_' . $blog_id, 'article_add'))
		{
			...
			while($task = mysql_fetch_array($result))
			{
				...
				$delete_confirm = ($task['system_task'] == '1') ? '' : 'onclick="javascript:if(!confirm(\''.addslashes(htmlspecialchars(get_lang("ConfirmYourChoice"),ENT_QUOTES,$charset)). '\')) return false;"';
		...
		while($assignment = mysql_fetch_array($result))
		{
			...
			echo	'<tr class="' . $css_class . '" valign="top">',
						 ...
						 '<td width="50">',
							...
							'onclick="javascript:if(!confirm(\''.addslashes(htmlspecialchars(get_lang("ConfirmYourChoice"),ENT_QUOTES,$charset)). '\')) return false;"',
							'<img src="../img/' . $delete_icon . '" border="0" title="' . $delete_title . '" />',
							"</a>\n",
						 '</td>',
					'</tr>';
