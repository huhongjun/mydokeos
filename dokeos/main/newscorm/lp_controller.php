...
// zml add ???è¦??????¥ç??è¯?è¨????ä»¶ç?????ç§?
if (isset($_GET['action']))
{ 
	if($_GET['action'] == 'export')
	{ //only needed on export ??????è¦?export
...
// zml edit ??? global.inc.php ä¹???? ???è¦?è®¾ç½®???è®¸å?¿å????¨æ?·ç?????è¯?
$use_anonymous = true;
...
// zml edit ???sessionå¼?å§???????ç±»ç??å®?ä¹?ä¹????ï¼?è¦?ç¡?ä¿?session ??? vars ä¸????å­?ä¹?è·?å¾?
...
if(!empty($_SESSION['refresh']) && $_SESSION['refresh']==1){
	...
	// zml edit æ£???¥å????????ä»?å¯?oLPå¯¹è±¡???äº???·æ?°ï??å¦???¨ç??è¾?LPä¹????ï¼?å¦??????·æ?°è??è®¾ç½®ï¼????ä»?ä»???°æ??åº??????????oLPå¯¹è±¡ï¼????ç§???·æ?°ï??
...
if(!empty($_REQUEST['dialog_box'])){
	...
	//stripslashes()??»æ????????çº¿å??ç¬??????½æ?°ç??ç±?: èµ????å¤????  è¿???????: å­?ç¬?ä¸?  è¯?æ³?: string stripslashes(string str);
	//???å®¹è?´æ??ï¼??????½æ?°å????»æ??å­?ç¬?ä¸²ä¸­?????????çº¿å??ç¬??????¥æ??è¿?ç»?äº?ä¸???????çº¿ï???????»æ??ä¸?ä¸?ï¼????ä¸?ä¸?ä¸??????¥å?????ä¸?ä¸???????çº¿ï??å°±ç?´æ?¥å?»æ?????
	
	//urldecode()è¿???? URL ç¼????å­?ç¬?ä¸? ??½æ?°ç??ç±?: ç¼????å¤???? è¿???????: å­?ç¬?ä¸? è¯?æ³?: string urldecode(string str);
	//???å®¹è?´æ??ï¼??????½æ?°å?? URL ç¼???????å­?ç¬?ä¸²è???????????ç¼?????????·å?????ç¼????ä½¿ç?? %## ?????¼å?????
}
...
if(isset($_SESSION['lpobject']))
{
	...
	//unserialize()å¯¹å??ä¸????å·²å?????????????????è¿?è¡????ä½?ï¼?å°???¶è½¬??¢å?? PHP ?????¼ã??
	//è¿??????????è½????ä¹?????????¼ï?????ä¸? integer???float???string???array ??? object???
	//å¦????ä¼???????å­?ç¬?ä¸²ä?????è§£å????????ï¼????è¿???? FALSE??? 
	if(is_object($oLP)){
		//is_object ??¤æ????????ç±»å????????ä¸ºç±»ç±»å????? è¯?æ³?: int is_object(mixed var); è¿???????: ??´æ?? ??½æ?°ç??ç±?: PHP ç³»ç???????? ???å®¹è?´æ??: ??¥å?????ä¸ºç±»ç±»å?????è¿????trueï¼???????è¿????false??? 
	...
	)
{
	...
	// zml edit ä¸?????????????é¡µé?¢é?½é??è¦???????ä¸?ä¸?lpå¯¹è±¡å¦?ä¸?ä¼?
	if(!empty($_REQUEST['lp_id']) || !empty($myrefresh_id)){
		...
		//right object
		// zml edit ??¨æ?°æ??åº?ä¸???????lpå¹¶æ????¥æ?????ç§?ç±»å??ï¼?scorm/dokeos/aiccï¼?ä»????äº§ç??æ­£ç¡®???å¯¹è±¡
...
/**
 * Actions switching  zml edit ???ä½?å¼????
 */
 // zml edit reinitialises ??°ç??è¢?JavaScript??¨æ?¥æ?´æ?°é¡¹???ä¸???????å½?
$_SESSION['oLP']->update_queue = array(); //reinitialises array used by javascript to update items in the TOC
 // zml edit åº?è¯¥ä½¿??? -> clear_message()??¹æ??ä½?ä¸?å·¥ä??
...
switch($_REQUEST['action'])
{
	case 'add_item':
		...
		{	//check if the learnpath ID was defined, otherwise send back to list
		    // zml edit æ£???¥å?????å­?ä¹?è·?å¾? ID è¢?å®?ä¹?äº????è¿??????°å??è¡?
			...
			{	//if a title was sumbitted 				
				...
				else
				{
					...
					{	//for all other item types than documents, load the item using the item type and path rather than its ID
						// zml edit å¯¹ä??????????¶ä??é¡¹ç??ç±»å????????ä»¶ç?¸æ??ï¼????è½½è?¥é¡¹???ä½¿ç?¨ç??é¡¹ç??ç±»å?????è·?å¾?ï¼??????????ID
	...
	case 'stats':
		...
		if(!$lp_found){ error_log('New LP - No learnpath given for stats',0); require('lp_list.php'); }
			...
			$stats_charset =  api_get_setting('platform_charset');//???å¹³å?°è?¾ç½®???ç¼????(gb2312)??¥è?¾ç½®å­?ä¹?è·?å¾???¶æ??é¡µé?¢ç?????:by xiaoping
