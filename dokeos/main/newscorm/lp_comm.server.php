...
function switch_item_details($lp_id,$user_id,$view_id,$current_item,$next_item)
{
	...
	*/
	...
	if($debug>1){error_log('Prereq_match() returned '.htmlspecialchars($mylp->error),0);}
	$objResponse->addScript("update_message_frame('".str_replace("'","\'",htmlspecialchars($mylp->error))."');");
