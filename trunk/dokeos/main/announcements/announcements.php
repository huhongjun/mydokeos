		...
		while ($myrow = Database::fetch_array($result))
		{
			if (!in_array($myrow['id'], $displayed))
			{
				...
				if(api_is_allowed_to_edit() OR (api_get_course_setting('allow_user_edit_announcement') && !api_is_anonymous()))
				{
					...
					if (api_is_allowed_to_edit()) echo "<td valign=\"top\"><a href=\"".api_get_self()."?".api_get_cidreq()."&action=delete&id=".$myrow['id']."\" onclick=\"javascript:if(!confirm('".addslashes(htmlspecialchars(get_lang('ConfirmYourChoice'),ENT_QUOTES,$charset))."')) return false;\">",
