<?php /*
for more information: see languages.txt in the lang folder. 
*/
$GeneralDescription = "概要简介";
$GeneralDescriptionQuestions = "课程在平台中的位置? 有无前提条件? 与其他的课程的联系?";
$GeneralDescriptionInformation = "描述这门课程(学时, 官方代码, 教室)和教师(名字, 姓氏, 办公室, 电话, 电子邮件...)";
$Objectives = "对象";
$ObjectivesInformation = "目标课程大体及详细地描述.";
$ObjectivesQuestions = "用户将在课程的最后做些什么呢? 课程进行中他们会作出什么样的创新呢";
$Topics = "话题";
$TopicsInformation = "主体列表包含在课程之中. 每个主题的重点. 难易程度. 结构和相互独立的不同部分.";
$TopicsQuestions = "课程的进程怎样? 用户需要特别注意的地方是哪里? 这些问题如果在不同的章节中理解方面会有什么区别吗? 用户在什么时间才能把精力放在不同的章节中呢?";
$Methodology = "方法学";
$MethodologyQuestions = "用什么样的方法才能有助于达到课程目标? 延什么样的进度表?";
$MethodologyInformation = "活动呈现(会议, 论文, 小组调查, 实验室...). ";
$CourseMaterial = "课程内容";
$CourseMaterialQuestions = "在
网络里是否有课本, 论文, 自传或链接列表?";
$CourseMaterialInformation = "课程内容的简短描述.";
$HumanAndTechnicalResources = "人力和技术资源";
$HumanAndTechnicalResourcesQuestions = "有教师, 助教, 技术支持, 社会主力, 以及计算机室吗?";
$HumanAndTechnicalResourcesInformation = "简要描述利用不同联系之间的人们和利用技术设备的的统一性.";
$Assessment  = "评估";
$AssessmentQuestions = "用户将如何评价? 用什么样的方法才能抓住主题?";
$AssessmentInformation = "举例测试问题. 正确标准. 提示以及窍门.";
?>