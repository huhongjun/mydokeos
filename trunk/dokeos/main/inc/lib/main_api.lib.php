...
function get_lang($variable, $notrans = 'DLTT')
{
	...
	// zml Global???ä½???¨æ??å®?ä¹???¨å????????,ä½????è¿?ä¸???¨å????????ä¸????åº???¨ä????´ä¸ªç½?ç«?,??????åº???¨ä??å½????é¡µé??,??????include???require????????????ä»?
	// ??»ç??:??¨å?½æ?°ä?????å®?ä¹????global??????,??½æ?°ä??å¤????ä»¥ä½¿???,??¨å?½æ?°ä??å¤?å®?ä¹????global??????ä¸???½å?¨å?½æ?°ä?????ä½¿ç??
...
function api_not_allowed($print_headers = false)
{
	...
	{//if the access is not authorized and there is some login information 
	 ...
	 // to the user_portal			
		...
		Display :: display_error_message(get_lang('NotAllowedClickBack').'<br/><br/><a href="javascript:history.back();">'.get_lang('BackToPreviousPage').'</a><br/>',false);
		echo '</div>';
