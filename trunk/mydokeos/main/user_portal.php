...
function get_personal_course_list($user_id)
{
	...
	$personal_course_list_sql = "SELECT course.code k, course.directory d, course.visual_code c, course.db_name db, course.title i, course.course_times cts, 
	...
	$personal_course_list_sql = "SELECT DISTINCT course.code k, course.directory d, course.visual_code c, course.db_name db, course.title i, course.course_times cts, course.tutor_name t, course.course_language l, 5 as s
	...
	$personal_course_list_sql = "SELECT DISTINCT course.code k, course.directory d, course.visual_code c, course.db_name db, course.title i, course.course_times cts, course.tutor_name t, course.course_language l, 2 as s
...
function get_logged_user_course_html($my_course)
{
	...
	$course_times = $my_course['cts'];//zml add courseTimes
	...
	if ($has_virtual_courses)
	{
		...
		$course_display_courseTimes = $return_result['course_times'];//zml edit
	}
	else
	{
		$course_display_title = $course_title;
		$course_display_code = $course_visual_code;
		$course_display_courseTimes = $course_times;//zml edit
	}
	...
	if ($course_visibility != COURSE_VISIBILITY_CLOSED || $user_in_course_status == COURSEMANAGER)
	{
		if(api_get_setting('use_session_mode')=='true' && !$nosession)
		{
			...
			{//zml ?????
				$result .= '<a href="'.api_get_path(WEB_COURSE_PATH).$course_directory.'/?id_session='.$my_course['id_session'].'">'.$course_display_title.'<font color=green>('.$my_course['course_time'].Count.')</font>'.'</a>';
			}
		}
		else
		{
...
else
{
	...
	foreach ($personal_course_list as $my_course)
	{
		...
		$thisCourseTime = $my_course['cts'];//zml edit
		$thisCoursePath = $my_course['d'];echo $my_course['cts'.'zml'];
